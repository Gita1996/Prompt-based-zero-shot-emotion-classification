...
def ensemble_strategy(data, name_dataset, output_file):
    ...
    for index, row in data.iterrows():
        ...
        list_pred = [row['emo_name'], row['expr_emo'], row['feels_emo'], row['emotion']]
        list_prob = [row['prob_emo_name'], row['prob_expr_emo'], row['prob_feels_emo'], row['prob_emotion']]
...
def oracle(data, name_dataset, output_file):
    ...
    y_pred = []
    for index, row in data.iterrows():
        label_gold = row['labels']
        if (label_gold == row['emo_name'] or label_gold == row['expr_emo'] or label_gold == row['feels_emo'] or label_gold == row['emotion']):
            TP += 1
            y_pred.append(label_gold)
        else:
            FN += 1
            y_pred.append(row['expr_emo'])
    t_ensemble = 'oracle'
...
def main():
    ...
    data['prob_emotion'] = data['prob_emotion'].apply(literal_eval)
